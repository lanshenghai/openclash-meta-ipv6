...
- [1. 仓库文件](#1-仓库文件)
- [2. 两种模式怎么选](#2-两种模式怎么选)
  - [2.1 实测参考](#21-实测参考同一-124mb-文件2026-07)
- [3. 模式 A：fake-ip + Redirect](#3-模式-afake-ip--redirectmeta-ipv6yaml)
  - [3.1 适用环境](#31-适用环境)
  - [3.2 核心结论](#32-核心结论一句话)
  - [3.3 推荐流量模型](#33-推荐流量模型)
  - [3.4 不要做的事](#34-不要做的事)
  - [3.5 OpenClash UCI](#35-openclash-uci模式-a)
- [4. 模式 B：redir-host + TUN](#4-模式-bredir-host--tunmeta-ipv6-tunyaml)
  - [4.1 为何需要 TUN](#41-为何需要-tun)
  - [4.2 yaml 配置要点](#42-yaml-配置要点)
  - [4.3 OpenClash UCI](#43-openclash-uci模式-b)
  - [4.4 TUN MTU](#44-tun-mtu无-uci-项)
  - [4.5 推荐流量模型](#45-推荐流量模型tun)
  - [4.6 客户端注意事项](#46-客户端注意事项)
  - [4.7 自检](#47-自检模式-b)
  - [4.8 回滚到模式 A](#48-回滚到模式-a)
- [5. 共用规则与 DNS 要点](#5-共用规则与-dns-要点)
  - [5.1 DNS](#51-dns两模式通用)
  - [5.2 rule-providers](#52-rule-providers保留集)
  - [5.3 rules 顺序](#53-rules-顺序摘要)
- [6. 部署步骤](#6-部署步骤)
  - [6.1 模式 A](#61-模式-a)
  - [6.2 模式 B](#62-模式-b)
- [7. 界面对照（UCI）](#7-界面对照uci)
- [8. License / 来源](#8-license--来源)
...
| 流量入口 | nft `redirect :7892` + IPv6 Redirect | **TUN `utun`**（yaml 含 `tun:` 段，OpenClash 启动时合并） |
...
## 3. 模式 A：fake-ip + Redirect（`meta-ipv6.yaml`）
...
## 4. 模式 B：redir-host + TUN（`meta-ipv6-tun.yaml`）
...
1. `meta-ipv6-tun.yaml` 已包含与运行时一致的 `tun:` 段（`stack: system`、`mtu: 1400` 等）；OpenClash 启动时仍可能合并 `dns-hijack` 等字段，以 `/etc/openclash/meta-ipv6-tun.yaml` 为准自检。
...
- **DNS 在路由器上**，不是 Windows 本机程序截获。截获发生在软路由的 dnsmasq → OpenClash。
- Windows Wi-Fi 若 IPv6 DNS 为 `fe80::1` 且查询超时，部分应用会解析失败。建议改为路由器 GUA（如 `240e:…::1`）或仅用 IPv4 DNS。
...
3. 按 §3.5 UCI 设置 fake-ip + IPv6 Redirect
...
2. 填入订阅与密钥（同上）
3. 按 §4.3 UCI 设置 `redir-host-tun` + `stack_type=system`
4. 运行 `scripts/patch-tun-mtu.sh 1400` 设置 MTU（若 yaml 已含 `mtu: 1400` 可跳过）
